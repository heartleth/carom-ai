...
function render(ctx, balls, center, n, alive) {
    if (dorender) {
        ...
        drawBall(ctx, 'orange', [center[0] + balls[0][0] * zoom, center[1] - balls[0][1] * zoom]);
        ...
        drawBall(ctx, 'white', [center[0] + balls[3][0] * zoom, center[1] - balls[3][1] * zoom]);
        ...
        ctx.strokeStyle = 'black';
        ctx.strokeText(n + (alive=='10'?'*':''), center[0] + (balls[3][0] - 6) * zoom, center[1] - (balls[3][1]) * zoom);
    }
}
...
function drawGraph(tt, gen) {
    ...
    for (let i = 0; i < 360; i++) {
        ctx.lineTo(center[0] * 2 * (i / 359), center[1] * 2 - tt[i] * 20);
    }
    ...
    for (let i = 0; i < 360; i++) {
        ctx.strokeStyle = 'white';
        ctx.beginPath();
        ctx.moveTo(center[0] * 2 * (i / 359), center[1] * 2);
        ctx.lineTo(center[0] * 2 * (i / 359), center[1] * 2 - tt[i] * 20);
        ctx.stroke();
        ctx.strokeStyle = 'grey';
        ctx.strokeText(tt[i], center[0] * 2 * (i / 59) - 8, center[1] * 2 - tt[i] * 20 - 10);
    }
}