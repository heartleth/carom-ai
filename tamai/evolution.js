...
class Entity {
    ...
    shoot() {
        this.life.input_values(this.ballcoords[0].concat(this.ballcoords[1]).concat(this.ballcoords[2]).concat(this.ballcoords[3]));
        this.life.evaluate();
        let out = this.life.binangle();
        let qstrength = this.life.strength() * 3 + 1;
        let theta = out * 3.1415926535897932 / 512;
        this.velocity = [[0, 0], [0, 0], [0, 0], [0, 0]];
        this.rolls = [[0, 0], [0, 0], [0, 0], [0, 0]];
...
let makenew = () => new Entity([rrandomw([8, 8]), rrandomw([8, 8]), rrandomw([12, 8])]);
...
let last = [];
let acacts = 0;
let scores = Array.from({ length: 360 }, ()=>0);
...
function randombetweenw(ad) {
    return Array.from({ length: ad[0].length }, (_, i) => randombetween(ad.map(e=>e[i]), 1));
}
